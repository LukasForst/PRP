...
void print_test(int *a, int a_len[], int *b, int b_len[], int *c, int c_len[],
                int i, char symbol1, char symbol2);
int main(int argc, char *argv[])
{
    ...
    for (;;) {
        ...
        *(matrix + matrix_counter) =
            (int *)calloc(rows * columns + 1000, sizeof(int));
    ...
    for (int i = 1; i <= matrix_counter; i++) {
        if (math_symbol[i] == '*') {
            if (size_of_matrix[i - 1][1] == size_of_matrix[i][0]) {
                ...
                int temp_size[2] = {size_of_matrix[i - 1][0],
                                    size_of_matrix[i][1]};
                while (mult_count + i <= matrix_counter &&
                       math_symbol[i + mult_count] == '*') {
                    if (temp_size[1] == size_of_matrix[i + mult_count][0]) {
                        temp_size[1] = size_of_matrix[i + mult_count][1];
                        max_rows =
                            temp_size[0] > max_rows ? temp_size[0] : max_rows;
                        max_columns = temp_size[1] > max_columns ? temp_size[1]
                                                                 : max_columns;
                        mult_count++;
                        continue;
                    } else {
                ...
                if (result_matrix_size[0] == temp_size[0] &&
                    result_matrix_size[1] == temp_size[1]) {
                    i += mult_count;
                    continue;
                } else {
    ...
    int *nula;
    nula = (int *)calloc(result_matrix_size[0] * result_matrix_size[1] + 1,
                         sizeof(int));
